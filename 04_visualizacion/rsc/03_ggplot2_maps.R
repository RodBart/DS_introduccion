...
data(USArrests)
head(USArrests)
ggplot(USArrests, aes(map_id = tolower(rownames(USArrests)))) +
...
bike <- read.csv("C:/Users/rodrigo/Documents/r4ds/04_visualizacion/dat/bike.csv")
head(bike)

mapa<-get_map( location = c(lon = -122.080954, lat = 36.971709),
         color = "color",
         maptype = "terrain",
         zoom = 14)
...
ggmap(mapa ) + 
  geom_path(aes(x = lon, y = lat, color = elevation),data = bike, size= 1.5)

...
largoFF <- melt(french_fries, id.vars = c("time","treatment","subject","rep"),
                  
                variable.name = "FF_variable", 
                value.name = "FF_value")
head(largoFF)

anchoFF<-dcast(largoFF, time+treatment+subject+rep ~ FF_variable, value.var = "FF_value")
head(anchoFF)


cuatro<- dcast(largoFF,treatment ~ FF_variable, fun.aggregate = mean,na.rm = TRUE, value.var ="FF_value" )

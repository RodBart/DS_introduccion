...
conteo<-Resul$count
muertos<-Resul$muertos
remove(muertos)

ggplot(Resul, aes(count,muertos)) +geom_point() + geom_smooth(method = "lm", se = FALSE) + labs(x = "Muertos ETA", y ="Muertos total")
...
En cuanto al número de muertos vamos a ver como la gráfica se dispara, dejando en algo casi anecdótico (si solo hacemos caso a los datos) los mueros en Europa occidental.
...
Y aquí se pueden ver los atentados mas grandes de cada año.
...
leaflet(max_anio) %>% addProviderTiles("OpenStreetMap.BlackAndWhite") %>% setView(0,0,1) %>%
    addCircles( lat = ~ latitude, lng = ~ longitude,weight = ~(maximo/40))
    

```


Y aquí se pueden ver los atentados mas grandes año a año.

```
```{r,echo=FALSE}
ui <- fluidPage(
  sliderInput("control", "Controller:", min=1970, max=2015, value=1970, step=1),
 tableOutput("tabla"),
  leafletOutput("mapa"
              #  ,width = "100%", height = "100%"
                ),
   tableOutput("control_tiempo")
)

server <- function(input, output, session)  {

  coordenadas<-reactive({
    if((length(max_anio[iyear==input$control]$iyear==input$control))==0) {max_anio[iyear==input$control-1]}else {max_anio[iyear==input$control]}
  })
  
    output$mapa <- renderLeaflet(
      leaflet(coordenadas()) %>% addProviderTiles("OpenStreetMap.BlackAndWhite") %>% setView(0,0,1) %>%
    addCircles( lat = ~ latitude, lng = ~ longitude,weight = ~(maximo/40))
    )

  
  Cambia_Input <- reactive({
  val <- if (input$control==2015) {1970} else {input$control}
  updateSliderInput(session, "control", value = val+1,  min = 1970, max = 2015, step =  1)
  })
  
  output$control_tiempo <- renderTable({

  Cambia_Input()
        Sys.sleep(2)
  })
  
    output$tabla <- renderTable({      max_anio[iyear==input$control, .(iyear, country_txt, maximo)]

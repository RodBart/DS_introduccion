...
Ed <- read.csv("http://tutorials.iq.harvard.edu/R/Rgraphics/dataSets/EconomistData.csv")
head(Ed)


  ggplot(Ed,aes(CPI,HDI.Rank,color = Region))+ geom_point() +
...
# para ajustar los textos, mira theme(strip.text = ...) y los argumentos de element_text
  ...
  ggplot(mpg, aes(drv,model))+ geom_point()+
    facet_grid(manufacturer ~ ., scales = "free_y")  + theme(strip.text.y= element_text( angle = 0))
